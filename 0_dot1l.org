...
We will then merge all these matrices/libraries into the same file with columns of:
 ...
 1. Library
 2. CellBarcode
 3. Dot1L Isoform Total
 4. Dot1L Isoform Specific Exon ...
 5. Dot1L Isoform Specific Exon ...
 6. Dot1L Isoform Specific Exon ...

The number of columns for "Dot1L Isoform Specific Exon" can be determined in advance, and I anticipate no more than 10.
...
* Download Datasets
   ...
   #+begin_src python :session yes :results output
     ...
     your_api_key = "2d71b7d06551df0023446673f8ed202a"
   ...
   #+end_example
    ...
    Just get the Dot1L transcripts
    ...
    Fix the names by prepping the matrices
    ...
    And now flatten the matrices into a table. (Let's pick one matrix and see how we can flatten it)
    ...
    #+begin_src R :session rare :var indir="" outdir="" :results output verbatim
      ...
      processMatrix <- function(matfilename, outfilename, outjunkname){
        ...
        trans.exon <- sapply(nonzero$gene, function(x){
          strsplit(
            strsplit(x, split="Dot1l\\|")[[1]][2],
            split="\\|ENS")[[1]][1]
        })
        ## all equal elements 
        trans.nomultiple.exons <- var(sapply(trans.exon, function (x) length(x))) == 0
        if (!trans.nomultiple.exons){
          stop("Multiple exons found in matrix ", matrixname)
        }
      ...
      sapply(input.matrices, function(x){
        matname <- gsub(x,
                        pattern=".*/([^_]+)_.*.tsv",
                        replacement="\\1")
    ...
    [1] "fixed_dot1l_longformat/C81744.dot1l.longformat.tsv 33"
    ...
    Error in var(sapply(trans.exon, function(x) length(x))) : 
      is.atomic(x) is not TRUE
    In addition: Warning message:
    In data.frame(trans = arr[[1]][1], exon = as.integer(arr[[1]][2])) :
      NAs introduced by coercion
    #+end_example

    #+begin_src elisp
      (format "We expect %d data lines in total" (+ 23 31 24 21 26 23 21 24 30 32 30 18 38 30 28 33 37 40 25 29 39 30 21 31 21 16 29))
    #+end_src

    #+RESULTS:
    : We expect 750 data lines in total

    Now let's cat them all together into one big matrix
    ...
    #+RESULTS:
    : 751 largemat/all.dot1l.longformat.tsv
